...
@Service
public class ChatMessageProcessor extends RegexMatchingProcessor {

    private final DiscordSendingService sendingService;

    @Autowired
    public ChatMessageProcessor(DiscordSendingService sendingService) {
        super("^(?:\\[.+\\] ){2}\\[[^\\<]+\\]\\: \\<(\\w+)\\> (.*)$");
    ...
    protected void processMatch(Mapping mapping, Matcher matcher) {
        ...
        try {
            sendingService.send(mapping, OutgoingDiscordMessage.userMessage(user, message));
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
